...
**Stack:** Next.js 14 App Router · TypeScript · Tailwind CSS · `node:sqlite` (Node 24 built-in, no native compilation)

**Database:** `mediaranker.db` is created in the project root on first run. The schema lives in `db/index.ts` which also exports all query helpers. All functions that return rows call `plain<T>()` (a `JSON.parse(JSON.stringify())` wrapper) — this is required because `node:sqlite` returns null-prototype objects that Next.js cannot serialize across the Server→Client boundary.
...
**webpack config:** `node:sqlite` is explicitly externalised in `next.config.js` so webpack doesn't attempt to bundle it. TMDB poster images require the `remotePatterns` entry for `image.tmdb.org` — removing it causes Next.js `<Image>` to 400 every poster.

## Key gotchas

- **`node:sqlite` row serialization** — always wrap query results in `plain<T>()` before passing to Client Components.
...
- **DB singleton** — `db/index.ts` uses `global._db` to survive Next.js hot-reload; don't open a second connection directly.
